#!/usr/bin/env python3
"""
SQLMap Runner - Automated tmux session launcher for SQL injection testing
Reads CSV from FINAL_FIX.py and creates tmux sessions with sqlmap
"""
...
def sanitize_session_name(domain):
    """Convert domain to valid tmux session name"""
    # Replace dots with underscores, remove special chars
    return domain.replace('.', '_').replace('-', '_').replace('/', '_')


def create_tmux_session(domain, vulns, prefix, sqlmap_path, dry_run=False, log_file=None):
    """
    Create tmux session for a domain with windows for each vulnerability
    ...
    # Build tmux commands and collect sqlmap commands for logging
    commands = []
    sqlmap_commands = []
    ...
    create_cmd = f"tmux new-session -d -s '{session_name}' -n '{window_name}' '{sqlmap_cmd}'"
    ...
    for vuln in vulns[1:]:
        ...
        new_window_cmd = f"tmux new-window -t '{session_name}' -n '{window_name}' '{sqlmap_cmd}'"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SQLMap Runner - Automated tmux session launcher')
    ...
    if args.log:
        with open(args.log, 'w', encoding='utf-8') as f:
            ...
            f.write(f"# Generated by tmuxrunner.py\n")
    ...
    for domain in selected_domains:
        ...
        # Create tmux session
        create_tmux_session(domain, top_vulns, args.prefix, args.sqlmap, dry_run=args.dry_run, log_file=args.log)
